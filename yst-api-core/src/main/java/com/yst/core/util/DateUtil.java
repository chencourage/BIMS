...
public class DateUtil {
    ...
    /**
     * 获取上一天凌晨23：59：59的时间毫秒数
     *
     * @return
     */
    public static Long getLastDayLast() {
        Calendar calendar = GregorianCalendar.getInstance();
        int year = calendar.get(Calendar.YEAR);
        int month = calendar.get(Calendar.MONTH);
        int date = calendar.get(Calendar.DATE);
        Calendar calendar2 = new GregorianCalendar(year, month, date - 1, 23, 59, 59);
        return calendar2.getTime().getTime();
    }
    /**
    ...
    /**
     * 获取当天凌晨23：59：59的时间毫秒数
     *
     * @return
     */
    public static Long getCurDayLast() {
        Calendar calendar = GregorianCalendar.getInstance();
        int year = calendar.get(Calendar.YEAR);
        int month = calendar.get(Calendar.MONTH);
        int date = calendar.get(Calendar.DATE);
     ...
     * @return
     */
    public static boolean isGreaterThan(Date date, Date target) {
        if (date == null || target == null) {
            log.error("判断有参数为空。[date={},target={}]", date, target);
            return false;
        }
        long stal = date.getTime();
        long endl = target.getTime();
        if (stal >= endl) {
            return true;
        } else {
            return false;
     ...
     * @return
     */
    public static boolean isLittleThan(Date date, Date target) {
        if (date == null || target == null) {
            log.error("判断有参数为空。[date={},target={}]", date, target);
            return false;
        }
    ...
    public static String msToTime(Date ms, String timeStyle) {
        if (ms == null || StringUtils.isBlank(timeStyle)) {
            return null;
        }
    ...
    public static String getFormattedYYYYmmddHHmmss(Date date) {
        SimpleDateFormat sdf = new SimpleDateFormat(DATE_STYLE_YYYYMMDDHHMMSS);
		return sdf.format(date);
    }
